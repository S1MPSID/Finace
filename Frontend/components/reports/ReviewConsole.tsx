...
export function ReviewConsole({ remarks, setRemarks, submitting, actionType, handleReview }: ReviewConsoleProps) {
  const canSubmit = remarks.trim().length >= 10;

  const onReview = (status: "verified" | "rejected") => {
    if (!canSubmit) return;
    handleReview(status);
  };
  ...
  return (
    <div className="glass rounded-[2rem] p-6 border-white/10">
      ...
      <textarea
        ...
        placeholder="Submit your expert remarks here (min. 10 characters)..."
        value={remarks}
        ...
        minLength={10}
      />
      {!canSubmit && remarks.length > 0 && (
        <p className="mb-3 text-xs text-amber-400/80">Remarks must be at least 10 characters for audit compliance.</p>
      )}
